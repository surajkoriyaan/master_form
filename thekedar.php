...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Add New Thekedar</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form action="thekedar.php" method="post">
    <div class="mb-3">
      <label  class="form-label">Enter Thekedar Name</label>
  ...
  <tbody>
   <?php
   $c=1;
   $sl="SELECT * FROM thekedar_master";
   ...
   if ($r==true) {
     while ($row=mysqli_fetch_assoc($r)) {
       ...
       <tr>
         ...
         <td>
     <a href="thekedar_update.php?sid=<?php echo $row['id']; ?>"><span class="badge bg-info text-dark">Update</span></a>
       <a href="thekedar_delete.php?sid=<?php echo $row['id']; ?>"><span class="badge bg-danger">Delete</span></a>
         </td>
       </tr>
       <?php
     }
...
if (isset($_POST["submit"])) {
  ...
  if (strlen($mobile1)==10 && strlen($mobile2)==10) {
    $inst="INSERT INTO thekedar_master(name,address,mobile1,mobile2)
    values('$name','$address','$mobile1','$mobile2')";
    if (mysqli_query($con,$inst)) {
      ?>
      <script type="text/javascript">
        alert("thekedar added successfully");
